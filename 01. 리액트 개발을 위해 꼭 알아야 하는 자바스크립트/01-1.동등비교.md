...
> 이전 글: [chapter 0. 왜 리액트인가?](https://github.com/kimyoungyin/react-deep-dive-gemini/blob/main/00-%EC%99%9C%EB%A6%AC%EC%95%A1%ED%8A%B8%EC%9D%B8%EA%B0%80/00.md)
...
-   이유1: JSX props는 기본적으로 객체이며, 이 첫 번째 깊이에 있는 값들만 일차적으로 비교하면 되기 때문(props vs props)
    ...
    // 부모 컴포넌트의 state가 변경되면 props.counter는 객체이므로 전후가 다른 참조값이기 때문에 이전 props와 이후 props가 다르게 판단되므로 리렌더링된다.
